...
4.打开tftpd32应用程序选择正确的ip（需要修改本机IP192.168.1.10），网线和串口都连接好板子打开mobaX选择正确的串口和波特率57600，命令2为重新烧录。输入板子ip与pc的ip处于同一网段（192.168.1.11），输入文件名test.bin通过tftpd完成烧录。每次串口连接需要断电后再通电。
5.拔掉网线后，打开mobaX选择正确的串口和波特率115200进入烧录好的系统。

实现摄像头：
修改rc.local文件使开机运行run_cam.sh文件，在ubuntu上运行vlc命令，即可将摄像头的内容推流到ubuntu的vlc终端显示。

注意：
1.每次修改openwrt文件都需要重新烧写。
2.开发板通过firewalll文件实现双向转发，四个lan口相同(bri-lan:192.168.1.1)，1个wan口(eth0.2:192.168.2.1)。
firewall配置参见https://blog.csdn.net/weixin_38387929/article/details/117828113?spm=1001.2014.3001.5506
firewall主要修改的是增加wan向lan的转发。
```
#----------change start----------------------
config forwarding
	option src		wan
...
#----------change end------------------------
```
3.与wan口相连的pc端网口应修改IP地址为192.168.2.2，默认网关设置为192.168.2.1；
与lan口相连的pc端网口应修改IP地址为192.168.1.2，默认网关设置为192.168.1.1；可能需要修改路由表。
4.该镜像目前存在问题lan口向wan口推流没有问题;wan口不能向lan口推流；双端都可以ping通。重新烧写看看效果。
